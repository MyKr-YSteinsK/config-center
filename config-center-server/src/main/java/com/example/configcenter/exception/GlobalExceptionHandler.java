...
public class GlobalExceptionHandler {
    ...
    public org.springframework.http.ResponseEntity<ApiResponse<?>> handleBiz(BizException e) {
        ...
        } else if (ec == ErrorCode.FORBIDDEN) {
    ...
    public org.springframework.http.ResponseEntity<ApiResponse<ErrorResponse>> handleBodyValidation(MethodArgumentNotValidException e) {
    ...
    public org.springframework.http.ResponseEntity<ApiResponse<?>> handleConstraint(ConstraintViolationException e) {
        return org.springframework.http.ResponseEntity.badRequest()
                .body(ApiResponse.fail(ErrorCode.PARAM_INVALID.getCode(), "请求参数错误: " + e.getMessage()));
    }

    @ExceptionHandler(MissingServletRequestParameterException.class)
    public org.springframework.http.ResponseEntity<ApiResponse<?>> handleMissingParam(MissingServletRequestParameterException e) {
        return org.springframework.http.ResponseEntity.badRequest()
                .body(ApiResponse.fail(ErrorCode.PARAM_INVALID.getCode(), "缺少参数: " + e.getParameterName()));
    }

    @ExceptionHandler(HttpMessageNotReadableException.class)
    public org.springframework.http.ResponseEntity<ApiResponse<?>> handleBadJson(HttpMessageNotReadableException e) {
        return org.springframework.http.ResponseEntity.badRequest()
    ...
    public org.springframework.http.ResponseEntity<ApiResponse<?>> handleConflict(DataIntegrityViolationException e) {
        return org.springframework.http.ResponseEntity.status(org.springframework.http.HttpStatus.CONFLICT)
                .body(ApiResponse.fail(ErrorCode.CONFLICT.getCode(), "数据冲突，可能是唯一键重复或并发写入导致"));
    }
    ...
    public org.springframework.http.ResponseEntity<ApiResponse<?>> handleOptimisticLock(Exception e) {
        return org.springframework.http.ResponseEntity.status(org.springframework.http.HttpStatus.CONFLICT)
