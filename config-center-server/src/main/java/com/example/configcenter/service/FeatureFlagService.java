...
public class FeatureFlagService {
    ...
    private final FeatureFlagHistoryRepository historyRepo;
    private final FeatureEvaluator evaluator = new FeatureEvaluator();

    public FeatureFlagService(FeatureFlagRepository repo, FeatureFlagHistoryRepository historyRepo) {
        this.repo = repo;
    ...
    public FeatureFlagDto upsert(UpsertFeatureRequest req) {
        ...
        if (!isCreate && req.getExpectedVersion() != null && ff.getVersion() != req.getExpectedVersion()) {
    ...
    public List<FeatureHistoryDto> history(String app, String env, String name) {
        return history(app, env, name, 50, null);
    }

    @Transactional(readOnly = true)
    public List<FeatureHistoryDto> history(
            String app, String env, String name, int limit, Long beforeVersion) {
        Pageable pageable = PageRequest.of(0, limit);
        List<FeatureFlagHistory> history = beforeVersion == null
                ? historyRepo.findAllByAppAndEnvAndNameOrderByVersionDesc(app, env, name, pageable)
                : historyRepo.findAllByAppAndEnvAndNameAndVersionLessThanOrderByVersionDesc(
                        app, env, name, beforeVersion, pageable);
        return history
                .stream()
    ...
    public FeatureEvalResult evaluate(String app, String env, String name, String userId) {
        ...
        if (!ff.isEnabled()) {
            return new FeatureEvalResult(name, userId, false, -1, "enabled=false（总开关已关闭）");
        }
        ...
        String decision = "灰度分桶：bucket=" + bucket + ", rollout=" + ff.getRolloutPercentage() + "%";
